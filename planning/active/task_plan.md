...
- [ ] New `R/fly_camera_patb.R` — `dest_dir = tempdir()` (never `fly_fetch()`'s `"photos"`
      ...
      **dedupes URLs** before fetching, unzips, dispatches on schema by column names
      ...
      uniqueness
- [ ] Join on `(tolower(roll), as.integer(frame))` — never on derived padding — and on
...
- [ ] Sweep `centroid_shapes()` — this attaches columns to user-supplied data, which is
      how #35 shipped through two releases (review G8)
- [ ] Test with the PAT-B rows **shuffled**, or a per-file implementation passes a
      per-frame test vacuously; and a frame absent from its archive returns `NA` rather
      than being dropped
- [ ] An archive with only `.ori`/`.ORI`, a schema with no identity column, or a URL that
...
- [ ] Measure the delivered thumbnail orientation for the three newly reachable cameras.
...
- [ ] Repoint the passages that become false, by name: `R/fly_footprint.R` `inferred_format`
...
- [ ] `NEWS.md` + `DESCRIPTION` bump to 0.10.0, as the final commit of the branch. NEWS
...
- [ ] End to end against the live population: of 41,249 frames, **22,366 resolve** —
