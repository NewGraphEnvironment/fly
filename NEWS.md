...
- `FLYING_HEIGHT` is metres above sea level, so subtracting terrain elevation is what turns it into the height ground coverage scales with. Elevation is sampled twice — at the centroid, then as the mean under the resulting rectangle, which differ by up to 140 m on a 7.2 km frame
