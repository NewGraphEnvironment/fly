...
**Repository:** NewGraphEnvironment/fly **Primary Language:** R
(package) **Extracted from:**
[airbc](https://github.com/NewGraphEnvironment/airbc) (issue \#16)

## Architecture

- One exported function per file: `R/fly_footprint.R` →
  `tests/testthat/test-fly_footprint.R`
- `inst/testdata/` — Upper Bulkley River floodplain near Houston, BC (20
  photos, dual scale). All 1968 film, `Film - BW`, focal 153 — so the
  *sampled* photos cannot exercise the digital path. The **AOI is not
  film-only**, though: it holds 181 `Digital - Colour` frames from two
  cameras, 24 of which now ship as
  `inst/testdata/photo_centroids_digital.gpkg` (fly#32). An earlier note
  here said digital coverage could not come from there; that came from a
  `BBOX(SHAPE, ...)` CQL query which returns 0 features for this AOI
  even for a positive control — a broken probe, not an absence. Query
  bboxes through `bcdata::filter(BBOX(...))`
...
- **Refuse rather than estimate an unknown recording format** (v0.4.0,
  ...
  while still drawing as a rectangle and still producing a coverage
  percentage.
  [`fly_footprint()`](https://newgraphenvironment.github.io/fly/reference/fly_footprint.md)
...
- **Every fixture in this package is plain `sf, data.frame`** — the
  ...
  `bcdata::collect()` returns `bcdc_sf, sf, tbl_df, tbl, data.frame`, so
  the suite is blind to anything that behaves differently on a tibble.
  That is how fly#35 shipped through two releases:
  [`sf::st_sf()`](https://r-spatial.github.io/sf/reference/sf.html)
  keeps only its first argument when that argument is a tibble, so
  `footprint_basis`, `footprint_terrain`, `height_agl` and
  `dem_coverage` reached no caller of the documented data source, with
  geometry and every downstream number still correct. Use
  `centroid_shapes()` in `tests/testthat/setup.R` — it sweeps plain /
...
- **[`fly_footprint()`](https://newgraphenvironment.github.io/fly/reference/fly_footprint.md)
  ...
  footprints in gives 100 rows out, silently. There is no guard yet
