...
- `R/fly_camera_format.R` — the digital sizing route: sensor dimensions per camera, keyed on
`camera_calibration_url`. `inst/extdata/camera_formats.csv` is generated by
`data-raw/make_camera_formats.R` from the calibration reports the catalogue links to, with
`camera_formats_excluded.csv` recording every calibration deliberately not shipped and why, and
...
- **The DEM is read through one window per frame, and that was measured rather than
assumed** (v0.13.0, #59) — `terra::extract()` handed the whole vector with no `fun` cost
...
a second call with a custom closure. **Per frame rather than once over the batch**, because
"crop once to what we are about to sample" is wrong form 4 of `dem_coverage` arriving through
the read — bounding it to one footprint costs 23% on contiguous frames and cannot produce the
...
- **Do not branch on `half_cross` / `half_along` in `fly_footprint()`.** They are `NA` for every
camera-table row until a sizing route fills them, and the routes run in sequence — so a condition testing
them before all routes have run is testing *arrival order*, not a property of the frame. This broke three
separate conditions over three review rounds in fly#32, each found inside the previous round's fix, and each
time the failure was total and silent for a whole class of frame. Branch on the recording format, which is
known before any route runs. See `inst/notes/camera-formats.md`
- **Before regenerating `inst/extdata/camera_formats.csv`, read `inst/notes/camera-formats.md`.** The
catalogue's `SCALE` is not the true image scale for a digital frame (it gives a third of true width), and
`GROUND_SAMPLE_DISTANCE` is in centimetres. The five QA checks in the generator are not interchangeable —
check B is the only one tight enough to catch a single wrong digit, and it is vacuous on rows where the
report states only two of pixel count, pitch and millimetres
...
- **Do not "speed up" the DEM read by cropping once to the batch** (fly#59) — that is the
