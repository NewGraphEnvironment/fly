...
- `R/fly_camera_format.R` — the digital sizing route: sensor dimensions
  per camera, keyed on `camera_calibration_url`.
  `inst/extdata/camera_formats.csv` is generated by
  `data-raw/make_camera_formats.R` from the calibration reports the
  catalogue links to, with `camera_formats_excluded.csv` recording every
  calibration deliberately not shipped and why, and
...
- `mixed_media_fixture()` in `tests/testthat/setup.R` — synthesized
  ...
  input shapes the invariant tests run over
...
- **Do not branch on `half_cross` / `half_along` in
  ...
  them, and the routes run in sequence — so a condition testing them
  before all routes have run is testing *arrival order*, not a property
  of the frame. This broke three separate conditions over three review
  rounds in fly#32, each found inside the previous round’s fix, and each
...
- **Before regenerating `inst/extdata/camera_formats.csv`, read
  ...
  and `GROUND_SAMPLE_DISTANCE` is in centimetres. The five QA checks in
